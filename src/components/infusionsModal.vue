...
function getSortedInfusions(infusionArray, sortKey) {
  ...
  // Calculate remaining percentage for sort by remainingMl
...
function printInfusionDetails() {
  ...
  if (selectedSortKey && selectedSortKey.value) {
    ...
    infusionDoc.content.push({alignment: 'left', text: `Infusion details, ${sortingtext}:`, fontSize: 18 })
  } else {
    infusionDoc.content.push({alignment: 'left', text: 'Infusion details:', fontSize: 18 })
  }
  ...
  const attributeLabels = {
    ...
    timeRemaining: 'Time remaining:', // we'll read from timeRemaining
    id: 'ID:',
    softwareVersion: 'Software Version:',
    medicalLibraryVersion: 'Medical Library Version:',
  }
  for (const item of displayedInfusions.value) {
    console.log(item)
